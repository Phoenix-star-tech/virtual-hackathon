...
export default function HomePage() {
  ...
  return (
    <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-16 xl:gap-20 items-center pb-16 lg:pb-28">
            <div className="lg:col-span-7 text-center lg:text-left space-y-6 lg:space-y-8">
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center lg:justify-start space-y-3 sm:space-y-0 sm:space-x-4 pt-2">
                <button onClick={() => setShowRegisterModal(true)} className="w-full sm:w-auto px-8 py-4 text-sm sm:text-base font-bold text-white bg-brand-blue hover:bg-brand-blueDark rounded-full shadow-lg shadow-brand-blue/20 transition-all duration-300 hover:-translate-y-0.5 text-center">
                  Register Now
                </button>
      ...
      {showRegisterModal && <RegisterModalView onClose={() => setShowRegisterModal(false)} onSelectModule1={() => { setShowRegisterModal(false); navigate("/register"); }} />}
...
function RegisterModalView({ onClose, onSelectModule1 }) {
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-[#111] border border-white/[0.08] rounded-2xl shadow-2xl p-8 sm:p-10 max-w-sm mx-4 w-full relative overflow-hidden" onClick={(e) => e.stopPropagation()}>
        ...
        <button onClick={onClose} className="absolute top-4 right-4 text-[#555] hover:text-[#aaa] transition-colors">
          <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" /></svg>
        </button>

        <div className="text-center mb-6">
          <h3 className="text-xl font-bold text-[#f0efec] tracking-tight">Choose a Module</h3>
          <p className="text-xs sm:text-sm text-[#666] mt-1">Select which module you want to register for</p>
        </div>

        <div className="space-y-3">
          <button
            onClick={onSelectModule1}
            className="w-full group flex items-center justify-between p-4 rounded-xl border border-white/[0.08] bg-white/[0.03] hover:bg-white/[0.06] hover:border-brand-blue/30 transition-all text-left"
          >
            <div className="flex items-center space-x-3">
              ...
              <div>
                <span className="text-sm font-bold text-[#f0efec] block">Module 1: Online Tasks</span>
                <span className="text-xs text-[#666] font-medium">₹9 registration — 20 days of coding tasks</span>
              </div>
            </div>
            <div className="text-brand-blue group-hover:translate-x-0.5 transition-transform">
          ...
          <div className="w-full flex items-center justify-between p-4 rounded-xl border border-white/[0.04] bg-white/[0.02] opacity-50 select-none cursor-not-allowed text-left">
            <div className="flex items-center space-x-3">
              <div className="w-10 h-10 rounded-xl bg-white/5 flex items-center justify-center text-[#555]">
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect width="18" height="11" x="3" y="11" rx="2" ry="2" /><path d="M7 11V7a5 5 0 0 1 10 0v4" /></svg>
              </div>
              <div>
                <span className="text-sm font-bold text-[#555] block">Module 2: 42h Online Sprint</span>
                <span className="text-xs text-[#444] font-medium">₹149/member — Team-based hackathon</span>
              </div>
            </div>
            <span className="text-[10px] font-bold uppercase text-[#555] bg-white/5 px-2 py-0.5 rounded">Locked</span>
          </div>
        </div>
